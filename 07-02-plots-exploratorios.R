...
datagr_prod <- datagr %>%
  ...
  group_by(Year, Protection_level, Region, Reef, sstmean, Degree, Habitat, Depth2, Transect, Diet, Family, Species) %>%
...
alldata <- datagr_prod |> 
  ...
  group_by(year, region, protection_level, habitat, reef, diet, depth2, transect, family)%>%
  summarise(biomass = sum(biom), product = sum(prod), turnover = sum(productivity))

ggplot(datagr_prod, aes(x = Prod, y = log(Biom), col = Diet)) +
...
alldata <- datagr_prod |> 
  janitor::clean_names() %>%
  # filter(region %in% c("Los Cabos", "Cabo Pulmo"), year > 2007) %>%
  group_by(year, region, sstmean, protection_level, habitat, reef, diet, depth2, transect, family)%>%
  summarise(biomass = sum(biom), product = sum(prod), turnover = sum(productivity))

glimpse(alldata)

p1 <- alldata %>%
  ggplot(aes(x= sstmean, y=biomass, col = region)) +
  # geom_point() +
...
alldata <- datagr_prod |> 
  janitor::clean_names() %>%
  group_by(year, region, reef, habitat, depth2, transect, family)%>%
  summarise(biomass = sum(biom), product = sum(prod), turnover = sum(productivity)
  )
...
ggsave("plot_family_spp_bot.png", legend_plot, width = 10, height = 5, dpi = 300)
